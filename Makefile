...
IMG ?= ratelimit-controller:latest
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/base/manager && $(KUSTOMIZE) edit set image ghcr.io/doodlescheduling/ratelimit-controller=${IMG}
...
kind-test: ## Deploy including test
	...
	kubectl --context kind-${CLUSTER} -n ratelimit-system delete pods --all
	kind load docker-image ${IMG} --name ${CLUSTER}
	kustomize build config/tests/cases/${TEST_PROFILE} --enable-helm | kubectl --context kind-${CLUSTER} apply --server-side=true -f -
	kubectl --context kind-${CLUSTER} -n ratelimit-system wait --for=condition=Ready pods -l control-plane=controller-manager -l app.kubernetes.io/managed-by!=Helm,verify!=yes --timeout=3m
	kubectl --context kind-${CLUSTER} -n ratelimit-system wait --for=jsonpath='{.status.conditions[1].reason}'=PodCompleted pods -l app.kubernetes.io/managed-by!=Helm,verify=yes --timeout=3m
...
golangci-lint: ## Download golint locally if necessary
	$(call go-install-tool,$(GOLANGCI_LINT),github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.13.1)
